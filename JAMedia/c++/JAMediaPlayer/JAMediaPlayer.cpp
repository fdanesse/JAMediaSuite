...
JAMediaPlayer::JAMediaPlayer(){
    ...
    video->add(gamma); video->add(videoflip); video->add(pantalla);

    convert->link(rate);rate->link(videobalance);videobalance->link(gamma);
    gamma->link(videoflip);videoflip->link(pantalla);
...
bool JAMediaPlayer::on_bus_message(const Glib::RefPtr<Gst::Bus>& bus,
    ...
    switch(message->get_message_type()){
            ...
            if (stateChangeMsg){
                ...
                if (estado != newState){
                    ...
                    if (estado == Gst::STATE_PLAYING){
                        ...
                        new_handler(false);}}}break;}

        case Gst::MESSAGE_EOS:{
            new_handler(false);signal_end.emit();return false;break;}

        case Gst::MESSAGE_ERROR:{
            Glib::RefPtr<Gst::MessageError> msgError =
                Glib::RefPtr<Gst::MessageError>::cast_static(message);
            if(msgError){Glib::Error err; err = msgError->parse();
                std::cerr << "Player Error: " << err.what() << std::endl;}
            else{std::cerr << "Player Error." << std::endl;}
            new_handler(false);return false;break;}
        ...
        case Gst::MESSAGE_TAG:{
            ...
            if (found!=std::string::npos){signal_video.emit();}break;}
...
void JAMediaPlayer::set_vol(double value){playbin->property_volume() = value;}
void JAMediaPlayer::play(){playbin->set_state(Gst::STATE_PLAYING);}

void JAMediaPlayer::pause_play(){
    //FIXME: Si esta stop y se da play no se reproduce a menos que se vuelva a presionar play
    if (estado == Gst::STATE_PLAYING){playbin->set_state(Gst::STATE_PAUSED);}
    else{playbin->set_state(Gst::STATE_PLAYING);}}

void JAMediaPlayer::stop(){
    new_handler(false); posicion = 0; playbin->set_state(Gst::STATE_NULL);
    signal_progress_update.emit(posicion);}

void JAMediaPlayer::load(Glib::ustring track, const gulong ventana_id){
    xid = ventana_id;posicion = 0;signal_progress_update.emit(posicion);
    //self.emit("loading-buffer", 100)
    if (gst_uri_is_valid(track.c_str())){
        playbin->property_uri() = track.c_str();progressbar = false;}
    else {playbin->property_uri() = Glib::filename_to_uri(track);
        progressbar = true;}}

void JAMediaPlayer::new_handler(bool reset){
    if (actualizador){actualizador.disconnect();}
    if (reset){actualizador = Glib::signal_timeout().connect(
        sigc::mem_fun(*this, &JAMediaPlayer::handler), 500);}}

bool JAMediaPlayer::handler(){
    if (progressbar){Gst::Format fmt = Gst::FORMAT_TIME;
        gint64 pos = 0; gint64 dur = 0;playbin->query_position(fmt, pos);
        playbin->query_duration(fmt, dur);
        pos /= Gst::SECOND; dur /= Gst::SECOND;pos = pos * 100 / dur;
        if (posicion != pos){posicion = pos;
            signal_progress_update.emit(posicion);}}return true;}

void JAMediaPlayer::seek_pos(gint64 val){
    Gst::Format fmt = Gst::FORMAT_TIME;
    gint64 pos = 0; gint64 dur = 0;playbin->query_position(fmt, pos);
    playbin->query_duration(fmt, dur);pos = dur * val / 100;
...
void JAMediaPlayer::rotar(Glib::ustring valor){
    int rot;videoflip->get_property("method", rot);
    if (valor == "Derecha"){if (rot < 3){rot += 1;}else{rot = 0;}}
    else if (valor == "Izquierda"){if (rot > 0){rot -= 1;}else{rot = 3;}}
    videoflip->set_property("method", rot);}

void JAMediaPlayer::set_balance(Glib::ustring prop, double valor){
    if (prop == "Brillo"){double val = (2.0 * valor / 100.0) - 1.0;
        videobalance->set_property("brightness", val);}
    else if (prop == "Contraste"){double val = 2.0 * valor / 100.0;
        videobalance->set_property("contrast", val);}
    else if (prop == "Saturacion"){double val = 2.0 * valor / 100.0;
        videobalance->set_property("saturation", val);}
    else if (prop == "Matiz"){double val = (2.0 * valor / 100.0) - 1.0;

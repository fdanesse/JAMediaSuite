...
JPaned::JPaned(){
    drawing = new Gtk::DrawingArea(); playerlist = new PlayerList();
    drawing->override_background_color(
        Gdk::RGBA("BLACK"), Gtk::STATE_FLAG_NORMAL);
    pack1(*drawing, Gtk::EXPAND); pack2(*playerlist, Gtk::FILL);
    show_all();
...
void JPaned::init(){playerlist->init();}
void JPaned::activar(Glib::ustring valor){playerlist->activar(valor);}
bool JPaned::motion(const GdkEventMotion *event){
    ...
    const int w = rect.get_width(); const int h = rect.get_height();
    bool ret = false;
    if (y < 41 or y > h - 41 or x > w - 41 ){ret = true;}
    else{ret = false;}
...
void JPaned::previous_track(){playerlist->previous_track();}
void JPaned::next_track(){playerlist->next_track();}
gulong JPaned::get_xid(){
    gulong xid = GDK_WINDOW_XID(drawing->get_window()->gobj());return xid;}
void JPaned::video(bool val){playerlist->video(val);} //FIXME: drawing debe estar oculto si no hay video
void JPaned::new_file(){playerlist->new_file();}

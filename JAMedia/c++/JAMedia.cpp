...
bool JAMedia::set_mouse(){
    ...
    if (tiempo - mov > 2){get_window()->set_cursor(cursor_blank);}
    return true;}

bool JAMedia::do_motion(const GdkEventMotion *event){
    time_t t = time(0);struct tm *now = localtime(&t);
    mov = (now->tm_hour * 60 * 60) + (now->tm_min * 60) + now->tm_sec;
    get_window()->set_cursor(jamedia_cursor);return true;}

void JAMedia::do_realize(){
    get_window()->set_cursor(jamedia_cursor);init();}
...
void JAMedia::toolbar_accion(Glib::ustring text, bool active){
    ...
    else if (text == "Lista"){panel->list_view(active);}
...
void JAMedia::motion(bool val){
    ...
    if (not view and not val){toolbar->hide();progress->get_parent()->hide();
        ...
        controls->show(); panel->list_view(vlis);}}
...
std::vector<std::basic_string<char> > JAMedia::run_open_files(){
    ...
    filter->set_name("Video");filter->add_mime_type("video/*");
    filter->add_pattern("*.mpg");filter->add_pattern("*.mp4");
    filter->add_pattern("*.mkv");filter->add_pattern("*.avi");
    filter->add_pattern("*.mpeg");filter->add_pattern("*.rmvb");
    filter->add_pattern("*.ogg");filter->add_pattern("*.ogv");
    dialog.add_filter(filter);filter = Gtk::FileFilter::create();
    ...
    dialog.add_filter(filter);int result = dialog.run();
    switch (result){case Gtk::RESPONSE_OK:lista = dialog.get_filenames();
        break;}dialog.hide();
    if (lista.size()){Glib::ustring path = lista[0].c_str();
...
void JAMedia::load_sub(){
    ...
    dialog.add_button("Abrir", Gtk::RESPONSE_OK);
    dialog.add_button("Cancelar", Gtk::RESPONSE_CANCEL);
    dialog.set_select_multiple(false); dialog.set_border_width(15);
    dialog.set_current_folder_uri("file://" + default_dirpath);
    Glib::RefPtr<Gtk::FileFilter> filter = Gtk::FileFilter::create();
    filter->set_name("Texto");filter->add_mime_type("text/*");
    dialog.add_filter(filter);int result = dialog.run();
    switch (result){case Gtk::RESPONSE_OK:file = dialog.get_filename();break;}
    dialog.hide();
    if (file.size()){default_dirpath = file.substr(
        0, file.find_last_of('/'));}player->load_sub(file);}

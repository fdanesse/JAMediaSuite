...
void ToolbarList::init(){
    con->set_active(false); con->set_sensitive(false);
    mas->set_sensitive(false); cle->set_sensitive(false);}

void ToolbarList::activar(Glib::ustring valor){
    size_t found;found = valor.find("clear");
    if (found!=std::string::npos){cle->set_sensitive(true);}
    found = valor.find("mas");
    if (found!=std::string::npos){mas->set_sensitive(true);}
    found = valor.find("con");
    if (found!=std::string::npos){con->set_sensitive(true);}}
...
void ToolbarList::clicked(Glib::ustring text){
    JAMedia *top = dynamic_cast<JAMedia*> (this->get_toplevel());
    if (text == "Clear"){top->init();}
    else if (text == "Open"){top->open_files();}
    else if (text == "Mas"){top->add_files();}}

void ToolbarList::video(bool val){
    con->set_active(false);con->set_sensitive(val);}

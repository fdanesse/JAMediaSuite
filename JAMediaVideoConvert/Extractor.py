...
class Extractor(Gst.Pipeline):
    ...
    def __on_pad_added(self, uridecodebin, pad):
        ...
        if string.startswith('audio/'):
            if self.codec == 'ogg':
                self.__set_audio_ogg_codec_bin()

            pad.link(self.audio_sink)

        elif string.startswith('video/'):
            if self.codec == 'ogg':
                self.__set_video_ogg_codec_bin()

            pad.link(self.video_sink)

    def __set_audio_ogg_codec_bin(self):
        """
    ...
    def __set_video_ogg_codec_bin(self):
        """
        Construye y Agrega los elementos necesarios
        para extraer el video en formato ogg.
        """
